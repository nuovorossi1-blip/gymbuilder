...
export default function Create() {
  ...
  function generateDay(session: WeeklySession, sourceProgram = weeklyProgram) {
    ...
    const workout = session.mode === 'crossfit'
      ...
      : session.mode === 'crossfit_hybrid' ? generaHybrid(dayCatalog, { ...common, priority_muscles: session.priority_muscles, method: session.priority_muscles.length > 0 ? 'specialization' : global.hybrid_method, format: session.variant === 'A' ? (global.hybrid_format ?? 'amrap') : 'emom' })
...
function WeeklyBuilder({ catalog, error, initial, onCreate }: { catalog: Exercise[]; error: string | null; initial: WeeklyProgramConfig; onCreate: (config: WeeklyProgramConfig) => void | Promise<void> }) {
  ...
  const toggleMode = (mode: PublicMode) => patch('selected_modes', config.selected_modes.includes(mode) ? config.selected_modes.filter((item) => item !== mode) : config.selected_modes.length < 2 ? [...config.selected_modes, mode] : config.selected_modes)
