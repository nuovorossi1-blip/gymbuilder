...
- [ ] Verificare con un utente autenticato reale (Create.tsx, non solo test/script) il giro completo dei fix del 19/08: FST-7 non più rigettato dal validatore (badge/esercizi visibili davvero nel wizard, non solo in `/avvia`), Push senza shoulder press fisso in 3ª posizione (dip come 4°), Legs con polpacci fisso e glutei come 6° slot, badge "Avvicinamento" prima del Top Set nel protocollo CBum.
- [ ] Se l'utente vuole ristrutturare anche Pull con uno slot "a più muscoli" come il dip in Push, serve un esempio concreto di esercizio (Pull non è stato toccato il 19/08 per mancanza di un esempio, a differenza di Push).
