...
- [ ] Verificare su telefono reale il secondo giro di fix Tabata (18/08): permesso notifiche chiesto una sola volta prima del countdown (`ensurePermission`), niente crash al primo passaggio lavoro→riposo, e resume corretto su "Avvia Tabata" dopo chiusura/riapertura (fix `setIniziato` mancante). Nessuno dei tre verificabile in questo Codespace.
